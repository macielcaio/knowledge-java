...
public class DesafioFor {
	...
	public static void main(String[] args) {
		...
		System.out.println("Informe a quantidade de alunos até 6: ");
		...
		for (int i = 0; i < notasAlunos.length; i++) {
			System.out.println("Informe a nota " + (i + 1) + ": "); //informa qual nota está sendo inserida
			notasAlunos[i] = entrada.nextDouble(); //cada linha será inserida uma nota, até a qunatida de alunos que foi setada
		}
		//Imprime os indices com cada valor que foi definido para cada qual
		System.out.println(Arrays.toString(notasAlunos));
		//soma todos os indices e calcula média
		double total = 0;
		for(double notas: notasAlunos ) {
			total += notas;
		}
		double media = total / notasAlunos.length;
		//Imprime os indices em forma de média
